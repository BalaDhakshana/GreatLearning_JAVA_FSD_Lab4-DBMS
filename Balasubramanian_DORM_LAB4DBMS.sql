...
create table supplier_pricing (PRICING_ID int,PRO_ID int,SUPP_ID int,SUPP_PRICE int default 0, constraint pricing_id_pk primary key(PRICING_ID),
...
/*Approach 2*/
Select cus_gender,count(distinct c.cus_id) as Total from customer c,order1 o where
 c.cus_id = o.cus_id and ord_amount >= 3000
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_type_of_service`()
...
CALL `ecommerce`.`sp_type_of_service`();
